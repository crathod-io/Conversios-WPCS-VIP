...
function add_message(type, msg, is_close = true) {
  ...
  if (type == "success") {
    document.getElementById("tvc_onboarding_popup_box").innerHTML =
...
function is_validate_step(step) {
  ...
  if (step == "step_1") {
    ...
    var ga4_account_id = "";
    var tracking_option = jQuery(
      "input[type=radio][name=analytic_tag_type]:checked"
    ).val();
    //console.log(tracking_option);
    if (tracking_option == "UA") {
      web_property_id = jQuery("#ua_web_property_id_option_val").attr(
        "data-val"
      );
      ua_account_id = jQuery("#ua_web_property_id_option_val").data(
        "accountid"
      );
      ...
      web_measurement_id = jQuery("#ga4_web_measurement_id_option_val").attr(
        "data-val"
      );
      ga4_account_id = jQuery("#ga4_web_measurement_id_option_val").data(
        "accountid"
      );
      ...
      web_property_id = jQuery("#both_ua_web_property_id_option_val").attr(
        "data-val"
      );
      ua_account_id = jQuery("#both_ua_web_property_id_option_val").data(
        "accountid"
      );
      web_measurement_id = jQuery(
        "#both_ga4_web_measurement_id_option_val"
      ).attr("data-val");
      ga4_account_id = jQuery("#both_ga4_web_measurement_id_option_val").data(
        "accountid"
      );
      //console.log(web_property_id+"=="+web_measurement_id);
      if (
        web_property_id == undefined ||
        web_property_id == "" ||
        web_measurement_id == undefined ||
        web_measurement_id == ""
      ) {
    ...
    if (google_ads_id == "") {
      msg =
        "You have not selected any google ads account. Please select it from the dropdown or create a new one to continue.";
...
jQuery(document).ready(function () {
  ...
  jQuery("input[type=checkbox]:not(#adult_content, #terms_conditions)").change(
    function () {
      //jQuery(".onbrdstep-1").removeClass('selectedactivestep');
      jQuery(".onbrdstep-3").removeClass("selectedactivestep");
      jQuery(".onbrdstep-2").removeClass("selectedactivestep");
...
function save_google_ga_ads_data(
  google_ads_id,
  tvc_data,
  subscription_id,
  tracking_option
) {
  var conversios_onboarding_nonce = jQuery(
    "#conversios_onboarding_nonce"
  ).val();
  if (subscription_id) {
    //GA start
    var web_measurement_id = "";
    ...
    if (tracking_option == "UA") {
      ...
      ua_account_id = jQuery("#ua_web_property_id_option_val").attr(
        "data-accountid"
      );
    } else if (tracking_option == "GA4") {
      web_measurement_id = jQuery("#ga4_web_measurement_id_option_val").attr(
      ...
      ga4_account_id = jQuery("#ga4_web_measurement_id_option_val").attr(
        "data-accountid"
      );
      ...
      ua_account_id = jQuery("#both_ua_web_property_id_option_val").attr(
        "data-accountid"
      );
      web_measurement_id = jQuery(
        "#both_ga4_web_measurement_id_option_val"
      ).attr("data-val");
      ga4_account_id = jQuery("#both_ga4_web_measurement_id_option_val").attr(
        "data-accountid"
      );
    ...
    if (is_valide != true) {
      ...
      if (web_property_id || web_measurement_id || google_ads_id) {
        data = {
          ...
          enhanced_e_commerce_tracking: jQuery(
          ...
          enhanced_link_attribution_tracking: jQuery(
          ...
          dynamic_remarketing_tags: jQuery("#dynamic_remarketing_tags").is(
            ":checked"
          ),
          google_ads_conversion_tracking: jQuery(
            "#google_ads_conversion_tracking"
          ).is(":checked"),
          link_google_analytics_with_google_ads: jQuery(
      ...
      jQuery.ajax({
        ...
        success: function (response) {
          ...
          if (response.error === false) {
            ...
            add_message(
              "success",
              "Google Analytics and Pixels successfully updated."
            );
            let tracking_option = jQuery(
              "input:radio[name=analytic_tag_type]:checked"
            ).val();
            var s_tracking_option = tracking_option.toLowerCase();

            if (
              jQuery("#link_google_analytics_with_google_ads").is(":checked")
            ) {
              if (tracking_option == "UA" || tracking_option == "BOTH") {
                if (tracking_option == "BOTH") {
                  s_tracking_option = "both_ua";
                }
                var profile_id = jQuery(
                  "#" + s_tracking_option + "_web_property_id_option_val"
                ).attr("data-profileid");
                var UalinkData = {
                    ...
                    jQuery.ajax({
                      ...
                      success: function (response) {
                        ...
                        if (response.error === false) {
                          add_message(
                            "success",
                            "Google Ananlytics and Google Ads linked successfully."
                          );
                        } else {
                        ...
                        if (
                          ...
                          var web_property = jQuery(
                            "#" +
                              s_tracking_option +
                              "_web_measurement_id_option_val"
                          ).attr("data-name");
                          var Ga4linkData = {
                            action: "link_analytic_to_ads_account",
                            type: "GA4",
                ...
                var web_property = jQuery(
                  "#" + s_tracking_option + "_web_measurement_id_option_val"
                ).attr("data-name");
                var Ga4linkData = {
                  action: "link_analytic_to_ads_account",
                  type: "GA4",
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        if (response.error === false) {
          ...
          add_message(
            "success",
            "Google Merchant Center account successfully updated."
          );
          //clearTimeout(tvc_time_out);
          var link_data = {
          ...
          if (google_merchant_center_id != "" && google_ads_id != "") {
            link_google_Ads_to_merchant_center(
              link_data,
              tvc_data,
              subscription_id
            );
...
function link_google_Ads_to_merchant_center(
  link_data,
  tvc_data,
  subscription_id
) {
        ...
        if (response?.data?.items.length > 0) {
          ...
          response?.data?.items.forEach(function (item) {
            AccOptions =
              ...
              '" ' +
              selected +
              'data-cat="accounts" data-accountid="' +
              item.id +
              '"> ' +
              item.name +
              "-" +
          ...
          jQuery("#ga4_account_id_option > .tvc-select-items").append(
            AccOptions
          ); //GA4
          jQuery("#ua_account_id_option > .tvc-select-items").append(
            AccOptions
          ); //GA3
          jQuery("#both_ua_account_id_option > .tvc-select-items").append(
            AccOptions
          ); //BOTH GA3
          jQuery("#both_ga4_account_id_option > .tvc-select-items").append(
            AccOptions
          ); //BOTH GA4
          ...
          add_message(
            "error",
            "There are no more Google Analytics accounts associated with this email."
          );
        } else {
          add_message(
            "error",
            "There are no Google Analytics accounts associated with this email."
          );
        }
      } else if (
        response &&
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      if (response && response.error == false) {
        var error_msg = "null";
        if (type == "GA4") {
          jQuery("#both_ga4_web_measurement_id_option_val").html(
            "Select Measurement Id"
          );
          jQuery("#both_ga4_web_measurement_id_option_val").attr(
            "data-val",
            ""
          );
          jQuery("#both_ga4_web_measurement_id_option_val").attr(
          ...
          jQuery(
            "#both_ga4_web_measurement_id_option > .tvc-select-items"
          ).html(""); //Both GA4
          ...
          if (response?.data?.wep_measurement.length > 0) {
            ...
            response?.data?.wep_measurement.forEach(function (item) {
              ...
              streamOptions =
                ...
                '<option value="' +
                item.measurementId +
                '" data-cat="dataStreams" data-name="' +
                dataName[1] +
                '" data-accountid="' +
                item.accountId +
                '">' +
            ...
            jQuery("#ga4_web_measurement_id_option > .tvc-select-items").append(
              streamOptions
            ); //GA4
            jQuery(
              "#both_ga4_web_measurement_id_option > .tvc-select-items"
            ).append(streamOptions); //BOTH
            jQuery("#tvc-ga4-web-edit_box").removeClass("tvc-disable-edits");
            jQuery("#both-tvc-ga4-web-edit_box").removeClass(
              "tvc-disable-edits"
            );
            ...
            add_message(
              "error",
              "There are no Google Analytics 4 Properties associated with this email."
            );
          }
        }
        if (type == "UA") {
          ...
          jQuery("#both_ua_web_property_id_option_val").html(
            "Select Property Id"
          );
          jQuery("#both_ua_web_property_id_option_val").attr("data-val", "");
          jQuery("#both_ua_web_property_id_option_val").attr(
            "data-profileid",
            ""
          );
          jQuery("#both_ua_web_property_id_option_val").attr(
          ...
          jQuery("#both_ua_web_property_id_option > .tvc-select-items").html(
            ""
          ); //BOTH GA3
          ...
          if (response?.data?.wep_properties.length > 0) {
            ...
            response?.data?.wep_properties.forEach(function (item) {
              PropOptions =
                ...
                '" data-profileid="' +
                item.id +
                '" data-cat="webProperties" data-accountid="' +
                item.accountId +
                '"> ' +
                item.webPropertyId +
                " - " +
            ...
            jQuery("#ua_web_property_id_option > .tvc-select-items").append(
              PropOptions
            ); //GA3
            jQuery(
              "#both_ua_web_property_id_option > .tvc-select-items"
            ).append(PropOptions); //BOTH
            jQuery("#tvc-ua-web-edit_box").removeClass("tvc-disable-edits");
            jQuery("#both-tvc-ua-web-edit_box").removeClass(
              "tvc-disable-edits"
            );
            ...
            add_message(
              "error",
              "There are no Google Analytics Properties associated with this email."
            );
          }
        }
      } else if (
    ...
    success: function (response) {
      ...
      if (response.error === false) {
        ...
        if (response.data.length == 0) {
          add_message(
            "warning",
            "There are no Google ads accounts associated with email."
          );
        } else {
          if (response.data.length > 0) {
            jQuery("#ads-account").append(
              jQuery("<option>", {
                value: "",
                text: "Select Google Ads Account",
              })
            ...
            jQuery.each(response.data, function (key, value) {
              if (selectedValue == value) {
                ...
                if (selectedValue == "" && key == 0) {
                  ...
                  jQuery("#ads-account").append(
                    jQuery("<option>", { value: value, text: value })
                  );
                }
              }
            });
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      ...
      if (response.error === false) {
        ...
        jQuery("#google_merchant_center_id").append(
          jQuery("<option>", {
            value: "",
            text: "Select Google Merchant Center",
          })
        );
        if (response.data.length > 0) {
          jQuery.each(response.data, function (key, value) {
    ...
    success: function (response) {
      if (response.error === false) {
        ...
        if (response.data.invitationLink != "") {
          jQuery("#ads_invitationLink").attr(
            "href",
    ...
    jQuery.ajax({
      ...
      success: function (response, status) {
        ...
        if (response.error === false || response.merchant_id != undefined) {
          ...
          add_message(
            "error",
            "There was error to create merchant center account"
          );
        }
        //user_tracking_data(btn_cam, error_msg,ename,event_label);
...
function user_tracking_data(
  ...
  jQuery.ajax({
    ...
    data: {
      ...
      TVCNonce:
        "<?php echo wp_create_nonce('update_user_tracking_data-nonce'); ?>",
    },
    success: function (response) {
