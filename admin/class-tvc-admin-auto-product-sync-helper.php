...
if (!class_exists('TVC_Admin_Auto_Product_sync_Helper')) {
  class TVC_Admin_Auto_Product_sync_Helper
  {
    ...
    */
    ...
    /*
     * Function used for real cron job to sync product (Manual/Auto)
     * Created By chirag virpariya 23-03-2023
     */
    public function cron_todo_product_sync($request)
    {
      ...
      try {
        ...
        // as_unschedule_all_actions('auto_feed_wise_product_sync_process_scheduler_ee',  array("feedId" => 1));
        ...
        if (!empty($ee_additional_data) && isset($ee_additional_data['is_mapping_update']) && $ee_additional_data['is_mapping_update'] == true) {
          ...
          if (!empty($mappedCats)) {
            ...
            foreach ($mappedCats as $mc_key => $mappedCat) {
              ...
              for ($i = 1; $i <= $total_page; $i++) {
                ...
                $TVC_Admin_Helper->plugin_log("Cron - category id " . $mc_key . " gmc product name " . $mappedCat['name'] . " - product count - " . isset($all_products) ? count($all_products) : 0, 'product_sync'); // Add logs
                if (!empty($all_products)) {
                  foreach ($all_products as $postvalue) {
                    $batch_count++;
                    array_push($values, esc_sql($postvalue), esc_sql($mc_key), esc_sql($mappedCat['id']), 1, date('Y-m-d H:i:s', current_time('timestamp')), 1);
                    $place_holders[] = "('%d', '%d', '%d','%d', '%s', '%d')";
                    if ($batch_count >= $product_db_batch_size) {
                      $query = "INSERT INTO `$prouct_pre_sync_table` (w_product_id, w_cat_id, g_cat_id, product_sync_profile_id, create_date, feedId) VALUES ";
        ...
        } else if (!empty($ee_additional_data) && isset($ee_additional_data['is_process_start']) && $ee_additional_data['is_process_start'] == true) {
          ...
          //$response = $TVCProductSyncHelper->call_batch_wise_auto_sync_product(); // Old call batch wise auto sync
          $response = $TVCProductSyncHelper->call_batch_wise_auto_sync_product_feed_ee(1);
          if (!empty($response) && isset($response['message'])) {
            $TVC_Admin_Helper->plugin_log("Cron - Batch wise auto sync process response " . $response['message'], 'product_sync');
          }

          $tablename = esc_sql($wpdb->prefix . "ee_prouct_pre_sync_data");
          $total_pending_pro = $wpdb->get_var("SELECT COUNT(*) as a FROM $tablename where `status` = 0 and `feedId` = 1");
        ...
        } else if (!empty($ee_additional_data) && isset($ee_additional_data['is_auto_sync_start']) && $ee_additional_data['is_auto_sync_start'] == true) {

          // Get again data to verify flags
          $ee_additional_data = $TVC_Admin_Helper->get_ee_additional_data();
          $ee_additional_data['product_sync_alert'] = NULL;
          $TVC_Admin_Helper->set_ee_additional_data($ee_additional_data);

          $timestamp = as_next_scheduled_action('ee_auto_product_sync_check');
          $nextDate = date("Y-m-d H:i:s", $timestamp);

          $tablename = esc_sql($wpdb->prefix . "ee_product_sync_data");
          $pendingCount = $wpdb->get_var("SELECT COUNT(*) as a FROM $tablename where status = 0 and `feedId` = 1");
          if ($nextDate <= date("Y-m-d H:i:s") || $pendingCount > 0) {
            as_unschedule_all_actions('ee_auto_product_sync_check');
            if ($pendingCount == 0) {
              $wpdb->query("UPDATE $tablename SET status = " . esc_sql(0));
              $last_sync_product_id = 0;
            } else {
              $productData = $wpdb->get_results($wpdb->prepare("select id from `$tablename` where status = 0 and `feedId` = 1 LIMIT 1"), OBJECT);
              if (!empty($productData) && isset($productData[0])) {
                $last_sync_product_id = $productData[0]->id;
              }
            }

            $this->TVC_Admin_Helper->plugin_log("Cron Auto - Start sync with last_sync_product_id=" . $last_sync_product_id, 'product_sync');
            $product_count = $this->TVC_Admin_DB_Helper->tvc_row_count('ee_product_sync_data');
            $pre_last_sync_product_id = sanitize_text_field($last_sync_product_id);
            if ($product_count > 0) {
              $tvc_currency = sanitize_text_field($this->TVC_Admin_Helper->get_woo_currency());
              ...
              if (!empty($products)) {
                ...
                if (!empty($p_map_attribute) && isset($p_map_attribute['items']) && !empty($p_map_attribute['items'])) {
                  ...
                  $this->TVC_Admin_Helper->plugin_log("Cron Auto - before product sync API Call for " . isset($p_map_attribute['items']) ? count($p_map_attribute['items']) : 0 . " products", 'product_sync');
                  $response = $this->customApiObj->products_sync($data);
                  $sync_status = 0;
                  if ($response->error == false) {
                    $sync_status = 1;
                  ...
                  // End call product sync API
                  $this->TVC_Admin_Helper->plugin_log("Cron Auto - after product sync API Call", 'product_sync');
                  $sync_product_ids = (isset($p_map_attribute['product_ids'])) ? $p_map_attribute['product_ids'] : "";
                  $last_sync_product_id = end($products)->id;
                  $total_sync_product = 0;
                  $action_scheduler_id = "";
                  ...
                  if ($pre_last_sync_product_id == 0) {
                    ...
                    $last_sync_row = $this->TVC_Admin_DB_Helper->tvc_get_last_row('ee_product_sync_call');
                    if (!empty($last_sync_row)) {
                      $countSync_product_ids = $sync_product_ids != "" ? count($sync_product_ids) : 0;
                      $total_sync_product = $countSync_product_ids + $last_sync_row['total_sync_product'];
                      $action_scheduler_id = $last_sync_row['action_scheduler_id'];
                      $next_sync = $last_sync_row['next_sync'];
                      $last_sync = $last_sync_row['last_sync'];
                    }
                  ...
                  $this->TVC_Admin_DB_Helper->tvc_add_row('ee_product_sync_call', $t_data, array("%s", "%d", "%d", "%s", "%s", "%s", "%d", "%d", "%d"));
                  //as_enqueue_async_action('ee_auto_product_sync_check', array('last_sync_product_id' => intval($last_sync_product_id) ));
                  $wpdb->query("UPDATE $tablename SET status = " . esc_sql(1) . " WHERE id <= " . $last_sync_product_id);
                  $this->TVC_Admin_Helper->plugin_log("Cron Auto - Done and update ee_auto_product_sync_check", 'product_sync');
                }
              }
            }
          }
