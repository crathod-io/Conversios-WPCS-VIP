...
if (!class_exists('TVCProductSyncHelper')) {
	class TVCProductSyncHelper
	{
		...
		public function __construct()
		{
			...
			$this->tiktok_business_id = sanitize_text_field($this->TVC_Admin_Helper->get_tiktok_business_id());
						...
						if ($prd->get_type() == "variable") {
							...
							if (!empty($p_variations)) {
								foreach ($p_variations as $v_key => $variation_id) {
									...
									$variableProduct->w_product_id = $variation_id;
									$variableProduct->w_cat_id = $postvalue->w_cat_id;
									$variableProduct->g_cat_id = $postvalue->g_cat_id;
				...
				if ($product_count > 0) {
					...
					if (!empty($products)) {
						...
						if (!empty($p_map_attribute) && isset($p_map_attribute['items']) && !empty($p_map_attribute['items'])) {
							...
							$this->TVC_Admin_Helper->plugin_log("Before product sync API Call for " . is_array($p_map_attribute['items']) ? count($p_map_attribute['items']) : 0 . " products", 'product_sync');
							$response = $CustomApi->products_sync($data);
			                      ...
			                      <div class="form-group">';
											...
											<div class="product_batch_size">
												<label>Product batch size
													<div class="tvc-tooltip">
														<span class="tvc-tooltiptext tvc-tooltip-right">If you are facing an issue with the product feed process with the current batch change the size of the batch according to your count of products.</span>
		...
		public function call_batch_wise_auto_sync_product_feed_ee($feedId){			
			...
			try {
				...
				if( $product_count > 0 ){
					...
					$filed = array( 'attributes', 'channel_ids', 'product_id_prefix', 'tiktok_catalog_id'
								);
					...
					$subscriptionId =  sanitize_text_field($this->subscriptionId);  
					$product_batch_size = esc_sql(intval($product_batch_size));
					$tiktok_business_id = $this->tiktok_business_id;
					...
					if(!empty($products)){
						...
						if(!empty($p_map_attribute) && isset($p_map_attribute['items']) && !empty($p_map_attribute['items'])){
							...
							$countProduct = is_array($p_map_attribute['items']) ? count($p_map_attribute['items']) : 0 ;
							$this->TVC_Admin_Helper->plugin_log("Product sync API Call for ".$countProduct , 'product_sync');
